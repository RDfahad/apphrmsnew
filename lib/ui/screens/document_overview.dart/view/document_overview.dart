...
class DocumentOverViewScreen extends StatelessWidget {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (state.documentData.documents?.isNotEmpty ?? false)
                        ListView.builder(
                            shrinkWrap: true,
                            itemCount: state.documentData.documents?.length ?? 0,
                              ...
                              return Container(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    _buildInfoRow(
                                        "Category:", state.documentData.documents?[index].category ?? ''),
                                    _buildInfoRow("Status:", state.documentData.documents?[index].type ?? ''),
                                    Row(
                                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                      children: [
                                        _buildInfoRow(
                                            "Type:", state.documentData.documents?[index].type ?? ''),
                                        Row(
                                          children: [
                                            InkWell(
                                                onTap: () async {
                                                  ...
                                                  downloadFile(context, state, index);
  ...
  void downloadFile(BuildContext context, DocumentState state, int index) {
    ...
    FileDownloader.downloadFile(
      ...
      onDownloadCompleted: (String path) {
        Navigator.pop(context); // Close the progress dialog when download completes
        log('FILE DOWNLOADED TO PATH: $path');
